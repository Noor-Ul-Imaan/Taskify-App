...
const EditOrg = () => {
  ...
  useEffect(() => {
    if (user) {
      setLoading(true);
      axios
        .get(`http://localhost:5000/organizations/${user._id}`)
        .then((response) => {
  ...
  const handleSaveOrg = async () => {
    ...
    try {
      ...
      setLoading(true);
      await axios.put(
        `http://localhost:5000/organizations/${user._id}`,
        requestData
      );
      setLoading(false);
    ...
    <div className="OrgSettings-container">
      ...
      {loading ? (
        ...
        <form>
                ...
                <li key={index} className="role-item">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      value={role.name}
                      onChange={(e) =>
                        handleRolesChange(index, "name", e.target.value)
                      }
                      required
                    />
          ...
          <button type="button" onClick={handleConfirmSaveChanges}>
            Save Organization
          </button>
        </form>
      )}
      ...
      <p className="delete-warning">
        Warning: All your organization information will be permanently deleted.
      </p>
      <button type="button" className="delete-button" onClick={handleDeleteOrg}>
        Delete Organization
      </button>
    </div>
  );
